...
def _work(process_id, model, dataset, args):
    ...
    with torch.no_grad(), cuda.device(process_id):
        ...
        for iter, pack in enumerate(data_loader):
            ...
            if args.irn4gcn:
                edge, dp = model(pack['img'][0].cuda(non_blocking=True), name=img_name, 
                                 path4feature4gcn=args.path4feature4gcn)
            else:
            ...
            if args.irn4gcn:
                rw = indexing.propagate_to_edge(cam_downsized_values, edge, beta=args.beta, exp_times=args.exp_times, radius=5, name=img_name, path4graph4gcn=args.path4graph4gcn)           
            else:
